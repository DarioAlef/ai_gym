...
# Verifica se o arquivo haarcascade existe, se não, faz o download
def setup_haarcascade():
    lib_dir = "lib"
    cascade_file = "haarcascade_frontalface_default.xml"
    cascade_path = os.path.join(lib_dir, cascade_file)
    
    # Cria o diretório lib se não existir
...
def setup_faces_directory():
    faces_dir = "faces"
    if not os.path.exists(faces_dir):
        os.makedirs(faces_dir)
        print("Diretório faces criado com sucesso!")

# Chama a função de setup
setup_faces_directory()
...
class FunctionScreen(Screen):
    ...
    def fotofaces(self, *args):
        ...
        def face_extractor(img):
            cascade_path = os.path.join("lib", "haarcascade_frontalface_default.xml")
            if not os.path.exists(cascade_path):
                print(f"Erro: Arquivo {cascade_path} não encontrado!")
                return None
        
            face_classifier = cv2.CascadeClassifier(cascade_path)
            if face_classifier.empty():
                print("Erro: Não foi possível carregar o classificador!")
                return None
            ...
            for (x, y, w, h) in faces:
                ...
                return cropped_face  # Retorna a primeira face encontrada
